...
const EducationCertificates = ({navigation}) => {
  const api = new Apis();
  ...
  const navigateButton = () => {
    if (!selectedImage) {
      Toaster({ message: `Please select certificate from the gallery.` });
      ...
      api.uploadWorkspaceDocuments(userData.id, 'QUALIFICATION_CERTIFICATE', form_data)
        .then(res => {
          // console.log("uploadWorkspaceDocuments", res);
          setIsLoading(false);
          Toaster({ message: `Certificate(s) submitted successfully.` });
          navigation.navigate('Verification', { uploaded: [...route.params.docs, 'doc-education-cert'] });
        }).catch(err => {
          setIsLoading(false);
          console.log(err);
        });
        ...
        <View style={[spacingProperty['m-15']]}>
          ...
          <View style={[spacingProperty['p-20']]}>
            ...
            <View style={[alignItemsCenter, spacingProperty['mb-20']]}>
              <Image
                source={require('../../../assets/educationCer.png')}
                style={{width: '100%'}}
              />
            </View>
            <View>
