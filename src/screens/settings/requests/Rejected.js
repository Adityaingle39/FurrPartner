...
export default RequestsRejected = ({navigation}) => {
	...
	const {appointments, rejectedRequests, setAppointments} = useAppointmentState();
  ...
  const [refreshing, setRefreshing] = useState(false);
	...
	const getAllAppointments = (type) => {
		...
		.then(res => {
			//console.log(`appointment all data ${JSON.stringify(res)}`);
			const responseData = res && res.length > 0 ? res : [];
			setAppointments(responseData);
			setRefreshing(false);
			setIsLoading(false);
			setAllAppointments(rejectedRequests(responseData));
		}).catch(err => {
	...
	useFocusEffect(
		React.useCallback(() => {
			if (!(defaultWorkspace.status == 'PENDING')) {
				setAllAppointments(rejectedRequests());
			}
		}, [])
	);
	...
	return (
		<SafeAreaView style={{flex:1, backgroundColor: colorScheme === 'dark' ? theme.colors.surface : theme.colors.surfaceVariant}}>
			<Header navigation={navigation} type='back' options={{title: 'Requests', subTitle: 'Rejected'}}></Header>
			<View style={{marginHorizontal: 10, marginTop: 10, marginBottom: allAppointments && allAppointments.length > 0 ? 105 : 0}} showsVerticalScrollIndicator={false}>
				{defaultWorkspace.status == 'PENDING' ? 
					...
					{allAppointments === null ? <ListAddress nos={7} /> : <VirtualizedList
						...
						ref={listRef}
						data={allAppointments ? allAppointments.sort((a, b) => new Date(b.appointmentDate) - new Date(a.appointmentDate)) : []} // Sorting appointments in descending order
						...
						ListHeaderComponent={<Text style={{ fontSize: 18, marginLeft: 5, color: theme.colors.onSurface }}>All Requests</Text>}
						ListEmptyComponent={<Empty style={{width: width - 30}} title="Empty!" subtitle="Bingo! You don't have any rejected requests." />}
					/>}
					</>
				}
			</View>
