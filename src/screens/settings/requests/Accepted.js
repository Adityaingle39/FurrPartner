...
export default RequestsAccepted = ({navigation}) => {
	...
	const {appointments, requestsAccepted, setAppointments} = useAppointmentState();
  ...
  const [refreshing, setRefreshing] = useState(false);
	...
	const getAllAppointments = (type) => {
		...
		.then(res => {
			...
			setRefreshing(false);
			setIsLoading(false);
			setAllAppointments(requestsAccepted());
		}).catch(err => {
	...
	return (
		<SafeAreaView style={{flex:1, backgroundColor: colorScheme === 'dark' ? theme.colors.surface : theme.colors.surfaceVariant}}>
			<Header navigation={navigation} type='back' options={{title: 'Requests', subTitle: 'Accepted'}}></Header>
			<View style={{marginHorizontal: 10, marginTop: 10, marginBottom: allAppointments && allAppointments.length > 0 ? 105 : 0}} showsVerticalScrollIndicator={false}>
				{defaultWorkspace.status == 'PENDING' ? 
					...
					{allAppointments === null ? <ListAddress nos={7} /> : <VirtualizedList
						...
						ListEmptyComponent={<Empty style={{width: width - 30}} title="Empty!" subtitle="Ooops! Currently you don't have any accepted requests." />}
					/>}
					</>
				}
			</View>
