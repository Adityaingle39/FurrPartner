...
export default RequestsCancelled = ({navigation}) => {
	...
	const {appointments, cancelledRequests, setAppointments} = useAppointmentState();
  ...
  const [refreshing, setRefreshing] = useState(false);
	...
	const getAllAppointments = (type) => {
		...
		.then(res => {
			...
			setRefreshing(false);
			setIsLoading(false);
			setAllAppointments(cancelledRequests());
		}).catch(err => {
	...
	return (
		<SafeAreaView style={{flex:1, backgroundColor: colorScheme === 'dark' ? theme.colors.surface : theme.colors.surfaceVariant}}>
			<Header navigation={navigation} type='back' options={{title: 'Requests', subTitle: 'Cancelled'}}></Header>
			<View style={{marginHorizontal: 10, marginTop: 10, marginBottom: allAppointments && allAppointments.length > 0 ? 105 : 0}} showsVerticalScrollIndicator={false}>
				{defaultWorkspace.status == 'PENDING' ? 
					...
					{allAppointments === null ? <ListAddress nos={7} /> : <VirtualizedList
						...
						ListEmptyComponent={<Empty style={{width: width - 30}} title="Empty!" subtitle="Ooops! Currently you don't have any cancelled requests." />}
					/>}
					</>
				}
			</View>
