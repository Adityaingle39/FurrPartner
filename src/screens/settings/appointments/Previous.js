...
export default AppointmentsPrevious = ({navigation}) => {
  ...
  const {appointments, previousAppointments, setAppointments} =
    useAppointmentState();
  ...
  const getAllAppointments = type => {
    api
      ...
      .then(res => {
        ...
        setRefreshing(false);
        setIsLoading(false);
        setAllAppointments(previousAppointments(responseData));
      })
      .catch(err => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      if (!(defaultWorkspace.status == 'PENDING')) {
        setAllAppointments(previousAppointments());
      }
    }, []),
  );
  ...
  return (
    <SafeAreaView
      ...
      <Header
        ...
        type="back"
        options={{title: 'Appointments', subTitle: 'Previous'}}></Header>
      <View
        style={{
          marginHorizontal: 10,
          marginTop: 10,
          marginBottom: allAppointments && allAppointments.length > 0 ? 105 : 0,
        }}
        ...
        {defaultWorkspace.status == 'PENDING' ? (
          <View style={{marginHorizontal: 30}}>
            <Empty
              ...
              subtitle={`We have recieved your request, it may take upto 2-3 working days to complete the verification process.\n\nContact our support team for further queries`}
            />
          </View>
        ) : (
          <>
              ...
              <VirtualizedList
                ...
                ListEmptyComponent={
                  <Empty
                    ...
                    title="Empty!"
                    subtitle="Ooops! Currently you don't have any previous appointments."
                  />
                }
              />
            )}
